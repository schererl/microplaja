...
DOMAIN_DIR="/home/mago/Desktop/PlaJa-RRL-Dataset/race_linetrack/non_det_icy_drop_park"
...
for p_name in "${PROBLEMS[@]}"; do
  for param in "${PARAMS[@]}"; do
    run_dir="${EVAL_DIR}/${p_name}_icy"
    mkdir -p "$run_dir"
    python -m testing_stuff.test_sym_model \
      --npz "${MODEL_DIR}/${p_name}_icy/${param}/pred_dump.npz" \
      --model "${MODEL_DIR}/${p_name}_icy/${param}/sym_model.json" \
...
for p_name in "${PROBLEMS[@]}"; do
  ...
  for param in "${PARAMS[@]}"; do
    sym_model="${MODEL_DIR}/${p_name}_icy/${param}/sym_model.json"
    python3 -m runner \
      --policy rrl \
      --sym_model "$sym_model" \
      --jani "$jani_file" \
