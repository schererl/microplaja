...
FNN="$MICRO_DIR/example/64@sym_model.json"
...
docker run --rm \
  ...
  -w /ws \
  "$IMAGE" \
  bash -c "mkdir -p build && cd build && make -j2"
  #bash -c "rm -rf build && mkdir -p build && cd build && ../cmake.sh build Release&& make -j2"
  #bash -c "mkdir -p build && cd build && make -j2"
